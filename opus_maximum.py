...
def main_window():
    ...
    def concordance_maker():
        ...
        instances = {item:(item, list(cur.execute(f'SELECT sentence, textname FROM entries WHERE lemma = "{item}"'))) for item in lemmata}
        show_prog(instances, 'Создание списка вхождений')
        instances = [instances[instance] for instance in instances]
        instances.reverse()
        ...
        for instance in instances:
            example_string = ''
            number = len(instance[1])
            example_string += f'#{instance[0]}=, '.upper()
            ...
            for pair in instance[1]:
                ...
                overall_string += example_string
            #text_body.insert(1.0, f'{example_string}')
            text_body.insert(1.0, f'{overall_string}')
...
#Выбор действия над составленной базой данных
    def whats_next():
        ...
        init_button = tkinter.Button(next_window, text = 'Поиск по слову', font = 'Arial 15', command=initial_search)
