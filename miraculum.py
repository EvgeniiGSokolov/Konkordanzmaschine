    ...
    def lemmatize(file):                                                               #НАЧАЛО БОЛЬШОЙ ФУНКЦИИ ЛЕММАТИЗАЦИИ
        ...
        def make_septet(entry):       #Эта функция нужна, чтобы из линейных грамм. данных с номером предложения получить список грамм. данных и текст предложения
            ...
            if len(quint) == 5:                                                                 #Проверяем, что наша упорядоченная пятёрка действительно пятёрка
                ...
                return quint[0],quint[1],quint[2],lemm_sent,orig_sent, len_sent, f'[{name}]'      #Возвращаем шестерку: (токен, лемма, грамм. хар-ка, предл-е, длина, название)
    ...
    def int_search():
        def do_search():
            ...
            def matcher(triple):
                ...
                sent = triple[1]
                name = triple[2]
                lemm_sequence = [item for item in re.split(' ',triple[0]) if item != '']
                if word_1 != '' and word_2 != '' and word_3 != '':
                    try:
                        w_1 = lemm_sequence.index(word_1)
                        w_2 = lemm_sequence.index(word_2)
                        w_3 = lemm_sequence.index(word_3)
                        real_dist_1 = fabs(w_1 - w_2) - 1
                        real_dist_2 = fabs(w_2 - w_3) - 1
                        if real_dist_1 == int(dist_1) and real_dist_2 == int(dist_2):
                            return(sent, name)
                    ...
                    pass
            results = sorted(set(x for x in (matcher(triple) for triple in to_check) if x != None))
            results = [f'{item[0]} {item[1]}' for item in results]
            overall_string = '\n'.join(results)
